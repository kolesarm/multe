...
4. EW: the efficiently weighted ATE estimator that runs one-treatment-at-a-time
   regression
5. CW: weighted ATE estimator using efficient common weights, as in Corollary 2
   in @ghk23.
...
Thus, the ATE estimator comparing `other` to `white` cannot be computed. The
...
To check whether there is a significant difference between PL and the
alternative estimators, the data frames `cb_f` and `cb_o` report differences
between the estimates, along with standard errors for the full and overlap
sample, respectively:
...
(i.e. significant contamination bias) for all races.

## Standard errors

The package also computes
"oracle" standard errors, in addition to the usual standard errors reported
above. These can be accessed in the `est_f` component of the output:
```{r r00}
print(m1$est_f, digits=3)
```

These oracle standard errors don't account for estimation error in the
propensity score, see Methods section below. Specifying the `cluster` argument
allows for computation of clustered standard errors:
...
\begin{equation}
  Y_{i}=\sum_{k=0}^{K}X_{ik}Z_{i}'\alpha_{k}+\dot{U}_{i}\label{eq:linear_mu},
    ...
    %   \label{eq:linear_tau}
\end{equation}
with $\dot{U}_{i}$ conditionally mean zero. The uninteracted estimator is given
by estimating
...
\eqref{eq:delta_system}, and $\ddot{X}_{ik}$ is the sample analog of
$\dbtilde{X}_{ik}$, the residual from regressing $X_{ik}$ onto $X_{i, -k}$ and
